...
class MessageAnalyser:
    ...
    def _streams_for(self, request_id: str | None) -> list[dict[str, Any]]:
        """Return the deduplicated cadence groups of the request's ledger."""
        try:
            ledger = self._ledger_for(request_id)
    ...
    def _build_prompt(
        ...
        return (
            ...
            f"RELATED_EVENT:\n{event_json}\n\n"
            f"REQUEST:\n{request_json}\n\n"
            "CANDIDATE_STREAMS (only valid stream_id values for tool 1):\n"
    ...
    async def _analyse_one(self, row: dict[str, Any]) -> _AnalysisOutcome:
        ...
        streams = self._streams_for(row["request_id"])
        try:
        ...
        try:
            decision = await self._connector_or_default().generate_structured(
                prompt,
                self._output_model,
        ...
        return _AnalysisOutcome(record, calls)
    ...
    def _ledger_for(self, request_id: str | None) -> ForecastLedger | None:
        if request_id is None:
            return None
        if request_id in self._ledgers:
            return self._ledgers[request_id]
        if self._forecaster is None:
            from ..cash_forecaster import CashForecaster

            self._forecaster = CashForecaster(store=self._store)
        try:
            ledger = self._forecaster.build_ledger(request_id)
        except KeyError:
            return None
    ...
    def _apply_call(
        ...
        try:
            ledger = self._ledger_for(record.request_id)
        except Exception as exc:  # noqa: BLE001 - record and continue the batch
            record.error = f"ledger build failed: {type(exc).__name__}: {exc}"
            return
        if ledger is None:
            record.error = "no forecast ledger for request; decision not applied"
            return
        try:
            record.affected = ledger.apply(tool, params)
        except ToolApplicationError as exc:
            record.error = f"tool not applied: {exc}"
            return
        record.applied = True
        record.ledger_request_id = record.request_id

    async def run(
        self, message_ids: Sequence[str] | None = None
    ) -> MessageAnalyserResult:
